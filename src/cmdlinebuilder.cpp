...
Regex regexMatchClassFBY("regexMatchClassFBY",
                 "^\\s*FBYCLASS\s*\(\s*(\w+)\s*\)");
...
void PrintSQLDefinition(ostream &ossql, ostream &oscpp, const string &currentClass, vector<MemberDescription> members)
{
    ...
    textToMember["int"] = "std::stoi";
    textToMember["long"] = "std::stol";
    textToMember["long"] = "long std::stoll";
    textToMember["float"] = "std::stof";
    textToMember["double"] = "std::stod";
    textToMember["long"] = "double std::stold";
    textToMember["string"] = "";
